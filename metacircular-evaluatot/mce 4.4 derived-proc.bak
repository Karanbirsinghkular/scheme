...
(define (if-alternative exp)
  (if (not (null? (cdddr exp)))
      (cadddr exp)
      'false));so if the if statement has no aternative then the value retutrned is false
...
(define (application? exp) (pair? exp 'call))
...
(define (eval-and exp env)
  (define (eval-and-proc ops)
    ((if (no-operands? ops)
      'true
      (make-if (eval (first-operand exp) env)
               (eval-and-proc (rest-operands exp))
               'false))))
  (eval-and-proc (operands exp)))

(define (or? exp)(tagged-list? exp 'or))
(define (eval-or exp env)
  (define (eval-or-proc ops)
    ((if (no-operands? ops)
      'false
      (make-if (eval (first-operand exp) env)
               'true
               (eval-or-proc (rest-operands exp))))))
  (eval-or-proc (operands exp)))
